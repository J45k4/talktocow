...
If Talktocow is served through nginx, raise nginx's default request body limit so browser image, recording, and video uploads can reach the backend. The app still enforces its own per-file upload limits.
