...
func testCoursesQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Courses().DeleteAll(ctx, tx); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testCourseToManyHomeworks(t *testing.T) {
	...
	if err = randomize.Struct(seed, &b, homeworkDBTypes, false, homeworkColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, homeworkDBTypes, false, homeworkColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Homeworks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := CourseSlice{&a}
	if err = a.L.LoadHomeworks(ctx, tx, false, (*[]*Course)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Homeworks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Homeworks = nil
	if err = a.L.LoadHomeworks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Homeworks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCourseToManyAddOpHomeworks(t *testing.T) {
	...
	var a Course
	var b, c, d, e Homework

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, courseDBTypes, false, strmangle.SetComplement(coursePrimaryKeyColumns, courseColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Homework{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, homeworkDBTypes, false, strmangle.SetComplement(homeworkPrimaryKeyColumns, homeworkColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Homework{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddHomeworks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Homeworks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Homeworks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Homeworks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	courseDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Description`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_             = bytes.MinRead
)
