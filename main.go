...
func main() {
	...
	r.POST("/api/files", routes.UploadFile)
	r.GET("/api/files/:fileId", routes.GetFile)
	r.DELETE("/api/files/:fileId", routes.DeleteFile)
	...
	r.GET("/api/diary/labels", routes.GetDiaryLabels)
